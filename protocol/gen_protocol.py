...
def emit_golden(c: dict) -> str:
    ...
    #
    # The scenario is a FACTORY-FRESH part — nothing bootable, so active_slot
    # is OB_SLOT_ID_NONE and the write target is slot A at the app base. That
    # keeps this vector coherent with the ERASE/WRITE/CRC vectors below, which
    # all address OB_APP_BASE, so section 14.2 reads as one session rather
    # than a device answering about one slot and a host writing another.
    #
    # CH592 slot geometry, derived the same way the Makefile derives it:
    # region 0x2000..0x70000 is 450560 bytes, so each slot is
    # floor(450560 / 2 / 4096) * 4096 = 225280 (0x37000) and slot B starts at
    # 0x39000. Capacity is one erase block less — the record owns the top
    ...
    hello_resp = (
        ...
        # --- 0.2: the A/B slot view ---
        + bytes([2, c["OB_SLOT_ID_NONE"], 0, 0])  # count, active, write, rsvd
        + u32(c["OB_APP_BASE"])  # write_base: slot A
        + u32(0x00036000)  # write_capacity
    )
    ...
    lines = [
        f"# OBP v{c['OB_PROTO_MAJOR']}.{c['OB_PROTO_MINOR']} golden vectors"
        " — generated by gen_protocol.py; do not edit.\n"
    ]
